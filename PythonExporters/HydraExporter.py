...
def ExportCMB(spec):
    ...
    '''
    manager = spec.getManager()
    analysis_name = spec.getAnalysisNames()[0]
    output_file_name = spec.getOutputPath();
    ...
    if analysis_name not in analysis_dict:
        print 'Unsupported analysis name \"%s\"" - no output generated' % \
            analysis_name
        return False

    categories = list(manager.analysisCategories(analysis_name))
    ...
    with open(output_file_name, 'w') as out:
        ...
        title = analysis_dict.get(analysis_name, 'unknown_analysis')
        out.write(title)
        out.write('\n')
...
def write_plotvar_section(manager, categories, out):
    '''
    Writes plotvar section for [Node/Elem/SideSet]PlotVarOutput attributes
    '''
    node_att_list = manager.findAttributes('NodePlotVarOutput')
    elem_att_list = manager.findAttributes('ElemPlotVarOutput')
    ss_att_list = manager.findAttributes('SideSetPlotVarOutput')
